...
def main() -> int:
    src = Path("out/gpt2_phase2d/phase_transition_2d.json")
    if not src.exists():
        print("ERROR: missing out/gpt2_phase2d/phase_transition_2d.json")
        print("Run first: python -m scripts.run_phase2d_gpt2")
        return 2
    ...
    ax.set_title("GPT2 Tiny: 3D Topography of Identity Emergence (lock_rate(w, λ))")
    ...
    print("PASS_GPT2_PHASE_TOPOGRAPHY_3D")
    print("")
    print("HUMAN TRANSLATION")
    print("This is the identity basin landscape for a real transformer backend (GPT2 tiny):")
    print("  x = sculpt strength w (logit bias toward target continuation tokens)")
    print("  y = constraint strength λ (soft penalty outside topic token set)")
